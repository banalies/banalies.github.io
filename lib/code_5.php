...
class EPAS_base {
    ...
    function EPAS_base($options = null) {
        ...
        if (isset($_COOKIE['sape_cookie']) && ($_COOKIE['sape_cookie'] == _EPAS_USER)) {
        ...
        if (!defined('_EPAS_USER')) {
            return $this->raise_error('Не задана константа _EPAS_USER');
        }
    ...
    function raise_error($e) {
        ...
        $this->_error = '<p style="color: red; font-weight: bold;">EPAS ERROR: ' . $e . '</p>';
 ...
 */
class EPAS_client extends EPAS_base {
    ...
    function EPAS_client($options = null) {
        parent::EPAS_base($options);
    ...
    function _get_dispenser_path() {
        return '/code.php?user=' . _EPAS_USER . '&host=' . $this->_host;
    }
 ...
 */
class EPAS_context extends EPAS_base {
    ...
    function EPAS_context($options = null) {
        parent::EPAS_base($options);
        $this->load_data();
    }
    ...
    function replace_in_text_segment($text) {
		/* преобразуем предложение в кодировку сайта */
		$text = $this->processReverseCharset($text);
    ...
    function _get_dispenser_path() {
        return '/code_context.php?user=' . _EPAS_USER . '&host=' . $this->_host;
    }

    function set_data($data) {
		/* запомним кодировку файла */
    	$this->detectSapeCharset($data);
        $this->_words = $data;
 ...
 */
class EPAS_articles extends EPAS_base {
    ...
    function EPAS_articles($options = null){
        parent::EPAS_base($options);
