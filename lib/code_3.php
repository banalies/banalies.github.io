...
class DeefknilCommon{
  ...
  function DeefknilCommon($options = null){
        if (!defined('DEEFKNIL_USER')) {
            return $this->raise_error("Constant DEEFKNIL_USER is not defined.");
        }
        $host = '';
...
class DeefknilClient extends DeefknilCommon{
    ...
    function DeefknilClient($options = null) {
        parent::DeefknilCommon($options);
        $this->load_links();
    ...
    /**
     *  function load_links(); определена в дочернем классе
     */
